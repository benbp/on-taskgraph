...
function mongoStoreFactory(waterline, Promise, Constants, Errors, _) {
    ...
    // NOTE: This MUST be called after updateDependentTasks, otherwise
    // the query should be:
    //     { $nin: [taskState, Constants.TaskStates.FinishedTaskStates] }
    // since we assume that any matching subdocuments would have been
    // removed already.
    function updateUnreachableTasks(data) {
        var query = {
            graphId: data.graphId,
        };
        query['dependencies.' + data.taskId] = data.taskState;
        var update = {
            $set: {
                reachable: false
            }
