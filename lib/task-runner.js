...
function taskRunnerFactory(
    ...
    function TaskRunner(options) {
        ...
        process.on('SIGINT', function() {
            ...
            console.log(Object.keys(self.activeTasks), ' active tasks');
            console.log(self.lostTasks, ' lost');
            return store.getOwnTasks(self.taskRunnerId)
            .then(function(tasks) {
                console.log(_.pluck(tasks, 'taskId'), ' owned Tasks');
            });
        });
    }
    ...
    TaskRunner.prototype.handleLostTasks = function() {
        ...
        return Rx.Observable.fromPromise(store.getOwnTasks(self.taskRunnerId))
            .flatMap(function(ownTasks) {
                logger.debug('handling lost tasks? ', {data: _.pluck(ownTasks, 'taskId')});
                ...
                return Rx.Observable.just(null);//empty();
            });
    };
