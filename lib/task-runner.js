...
function taskRunnerFactory(
    ...
    TaskRunner.prototype.createRunTaskSubscription = function(runTaskStream) {
        ...
        return runTaskStream
            ...
            .flatMap(safeStream.bind(self, store.checkoutTaskForRunner))
            .filter(function(data) { return !_.isEmpty(data);})
            .flatMap(safeStream.bind(self, store.getTaskById))
            .flatMap(safeStream.bind(self, self.runTask))
            .subscribe(
    ...
    function safeStream(toObserve, streamData) {
        var self = this;
        return Rx.Observable.just(streamData)
            .flatMap(toObserve)
            .catch(self.handleStreamError.bind(self, 'Some kind of Error'));
    }
