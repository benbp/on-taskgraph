...
function taskRunnerFactory(
    ...
    TaskRunner.prototype.cancelTask = function(data) {
        ...
        return Rx.Observable.just(data)
            ...
            .flatMap(function(task) { return task.cancel(new Errors.TaskCancellationError()); })
            .finally(function() {
    ...
    TaskRunner.prototype.createHeartbeatSubscription = function() {
        ...
        return Rx.Observable.interval(self.heartbeatInterval)
                ...
                .flatMap(safeStream.bind(self,
                            self.stopUnownedTasks.bind(self),
    ...
    TaskRunner.prototype.stopUnownedTasks = function() {
        var self = this;
        return Rx.Observable.fromPromise(store.getOwnTasks(self.taskRunnerId))
            .map(function(stuff) {
                ...
                _.difference(Object.keys(self.activeTasks), _.pluck(stuff, 'taskId'))
                    .forEach(function(taskId) {
                        console.log('Stopping and resetting task', taskId);
    ...
    TaskRunner.prototype.runTask = function(data) {
        ...
        return Rx.Observable.just(data)
            ...
            .tap(function() {
                ...
                console.log("DONE TASKS ", self.completedTasks);
            })
            .finally(function() {
