...
function factory(Task, scheduler, registry, eventsProtocol, schedulerProtocol,
    ...
    function TaskGraph(definition, context) {
        ...
        //this._status = 'valid';
        this._status = TaskStates.Pending;
    ...
    TaskGraph.prototype.getTaskStates = function() {
        ...
        _.forEach(self.tasks, function(task) {
            var summary = { id: task.instanceId, name: task.name };
            if (task.state === TaskStates.Pending) {
                result.pending.push(summary);
            } else if (task.state === TaskStates.Running) {
                result.running.push(summary);
            } else if (_.contains(self.finishedStates, task.state)) {
                result.finished.push(summary);
            }
        });
        return result;
    };

    TaskGraph.prototype.getPendingTasks = function() {
        return _.compact(_.map(this.tasks, function(task) {
            if (task.state === TaskStates.Pending) {
                return { id: task.instanceId, name: task.name };
            }
        }));
    };

    TaskGraph.prototype.getRunningTasks = function() {
        return _.compact(_.map(this.tasks, function(task) {
            if (task.state === TaskStates.Pending) {
                return { id: task.instanceId, name: task.name };
            }
    ...
    TaskGraph.prototype.getFinishedTasks = function() {
        var finishedStates = this.finishedStates;
        return _.compact(_.map(this.tasks, function(task) {
            if (_.contains(finishedStates, task.state)) {
                return { id: task.instanceId, name: task.name };
            }
        }));
    };

    TaskGraph.prototype.getNumFinishedTasks = function() {
        var finishedStates = this.finishedStates;
        var count = 0;
    ...
    TaskGraph.prototype._findReadyTasks = function () {
        var self = this;
        _.forEach(self.getPendingTaskObjects(), function(task) {
    ...
    TaskGraph.prototype._scheduleReadytasks = function() {
        ...
        while (this.ready.length > 0) {
            ...
            logger.debug("Scheduling task", {
                ...
                remainingTasks: this.getPendingTasks()
            });
    ...
    TaskGraph.prototype._checkDone = function() {
        ...
        var finishedTaskCount = this.getNumFinishedTasks();
    ...
    TaskGraph.prototype.serialize = function serialize() {
        var self = this;
        var taskStates;
        var omit = ['tasks', 'subscriptions', '_events', 'ready', 'finishedStates', 'failedStates'];

        var obj = _.transform(this, function(result, v, k) {
            if (!_.contains(omit, k)) {
                result[k] = v;
            }
        }, {});
        ...
        obj.pendingTasks = taskStates.pending;
        obj.runningTasks = taskStates.running;
        obj.finishedTasks = taskStates.finished;
