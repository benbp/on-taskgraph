...
function taskGraphFactory(
    ...
    TaskGraph.prototype.constructTaskObject = function(taskName, taskOverrides,
           optionOverrides, label) {
        var self = this;
        return taskdefinitions.getTask(taskName)
        ...
        .then(function() {
            return Promise.map(self.definition.tasks, function(taskData) {
                if (_.has(taskData, 'taskDefinition')) {
                    ...
                    return taskdefinitions.getTask(taskData.taskName)
                    .then(function(taskDefinition) {
                        return Promise.all([
    ...
    TaskGraph.prototype._getBaseTask = function(definition) {
        ...
        return taskdefinitions.getTask(definition.implementsTask)
        .then(function(baseTaskDefinition) {
    ...
    TaskGraph.create = function (definition, options, context) {
        // TODO: make separate factory function instead of discovering possible
        // failures with getting the store on the first time we call create.
        if (!taskdefinitions) {
            taskdefinitions = store.getTaskDefinitionStore();
        }
