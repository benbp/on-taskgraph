...
function taskSchedulerFactory(
    ...
    TaskScheduler.prototype.createGraphDoneSubscription = function(readyTaskStream) {
        ...
        return readyTaskStream
                ...
                .map(function(data) { return _.pick(data, ['graphId', 'schedulerId']); })
                .subscribe(
    ...
    // TODO: refactor to stream-based
    TaskScheduler.prototype.publishGraphDone = function(data) {
        return store.setGraphDone(data.graphId, Constants.TaskStates.Succeeded)
        .then(function(graph) {
            if (!_.isEmpty(graph)) {
                // TODO: mark graph state as done here in the db?
                // TODO: use Rx, abstract the messenger strategy away to another module.
                return eventsProtocol.publishGraphFinished(data.graphId, graph._status);
            }
        })
        .then(function() {
            return data;
