...
function taskSchedulerFactory(
    ...
    function TaskScheduler() {
        ...
        this.subscriptions = [];
        this.streamHandlers = [];
        this.graphStateStream = new Rx.Subject();
        this.externalContextStream = new Rx.Subject();
        this.pipeline = null;
    }

    TaskScheduler.prototype.initializePipeline = function() {
        var schedulerStream = Rx.Observable.merge(
            this.graphStateStream.map(this.evaluateGraphStateHandler.bind(this)),
            this.externalContextStream.map(this.evaluateExternalContextHandler.bind(this))
        );

        return schedulerStream.subscribe(this.scheduleTaskHandler.bind(this));
    };
    ...
    TaskScheduler.prototype.handleRunTaskGraphRequest = function(name, options, target) {
        ...
        this.graphStateStream.onNext(graph);
