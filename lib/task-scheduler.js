...
function taskSchedulerFactory(
    ...
    TaskScheduler.prototype.getConcurrencyMaximums = function(concurrentOptions) {
        ...
        var _options = _.defaults(concurrentOptions || {}, {
            ...
            checkTaskStateHandledByGraph: 100,
            findReadyTasks: 100,
            handleFailGraphEvent: 100,
    ...
    TaskScheduler.prototype.initializePipeline = function() {
        ...
        this.createUpdateTaskDependenciesSubscription(
                taskHandlerStream, this.evaluateGraphStream, this.checkGraphFinishedStream);
        this.createTasksToScheduleSubscription(this.evaluateGraphStream);
        this.createGraphFinishedSubscription(this.checkGraphFinishedStream);
    };
    ...
    TaskScheduler.prototype.checkTaskStateHandledByGraph = function(data) {
        ...
        .flatMap(function(data) {
            if (_.contains(Constants.FailedTaskStates, data.state) &&
                _.contains(data.terminalOnStates, data.state)) { data.unhandledFailure = true;
            } else {
    ...
    TaskScheduler.prototype.createGraphFailSubscription = function(taskHandlerStream) {
        ...
        .map(function(data) {
            return _.pick(data, ['domain', 'graphId', 'taskId', 'state']);
        })
        .subscribe(
    ...
    TaskScheduler.prototype.handleFailGraphEvent = function(data) {
        ...
        .flatMap(store.setTaskStateInGraph.bind(store, data))
        .flatMap(function() {
        ...
        .subscribe(
            function(data) {
                if (_.contains(data.terminalOnStates, data.state)) {
                    checkGraphFinishedStream.onNext({ graphId: data.graphId });
                } else {
    ...
    TaskScheduler.prototype.createGraphFinishedSubscription = function(checkGraphFinishedStream) {
        var self = this;
    ...
    TaskScheduler.prototype.createUnevaluatedTaskPollerSubscription = function(evaluateTaskStream) {
        ...
        .takeWhile(self.isRunning.bind(self))
        .map(function() {
            var queueSize = (self.concurrencyMaximums.checkTaskStateHandledByGraph.max -
                            self.concurrencyMaximums.checkTaskStateHandledByGraph.count);
            // Don't poll if we're above 50% capacity
            if (queueSize < self.concurrencyMaximums.checkTaskStateHandledByGraph.max / 2) {
                return Rx.Observable.empty();
            }
            return self.findUnevaluatedTasks(self.domain,
                (self.concurrencyMaximums.checkTaskStateHandledByGraph.max -
                    self.concurrencyMaximums.checkTaskStateHandledByGraph.count));
        })
