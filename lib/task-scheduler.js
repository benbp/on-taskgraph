...
function taskSchedulerFactory(
    ...
    TaskScheduler.prototype.initializePipeline = function() {
        var tasksToSchedule = this.evaluateGraphStream
                ...
                .flatMap(function(tasks) {
                    ...
                    return Rx.Observable.from(tasks || []);
                })
                .flatMap(store.checkoutTaskForScheduler.bind(store, this.schedulerId))
    ...
    // to stick a promise chain in here when everything else is simplified
            ...
            if (_.isEmpty(tasks)) {
                ...
                .then(function(result) {
                    if (result) {
                        ...
                        eventsProtocol.publishGraphFinished(result.instanceId, result._status);
                    }
                });
            } else {
                return tasks;
