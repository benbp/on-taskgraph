...
function taskSchedulerFactory(
    ...
    TaskScheduler.prototype.initializePipeline = function() {
        ...
        this.createHeartbeatSubscription();
    };
    ...
    TaskScheduler.prototype.findReadyTasks = function(data) {
        ...
        .flatMap(function(_data) {
            return store.findReadyTasks(
                self.schedulerId, self.domain, _data.graphId);
        })
    ...
    TaskScheduler.prototype.createUpdateTaskDependenciesSubscription =
        ...
        .subscribe(
            function(graphId) {
                evaluateGraphStream.onNext({ graphId: graphId });
            },
    ...
    TaskScheduler.prototype.createTasksToScheduleSubscription = function(readyTaskStream) {
        ...
        .tap(function(task) {
            var _task = _.pick(task, ['domain', 'graphId', 'taskId']);
            self.handleStreamDebug('Recieved schedule task event', _task);
        })
    ...
    TaskScheduler.prototype.createHeartbeatSubscription = function() {
        var self = this;
        return Rx.Observable.interval(self.heartbeatInterval)
                .takeWhile(self.isRunning.bind(self))
                .flatMap(store.heartbeatTasksForScheduler.bind(self, self.schedulerId))
                .subscribe(
                    self.handleStreamSuccess.bind(self, null),
                    function(error) {
                        logger.error('Failed to update heartbeat, stopping scheduler and tasks', {
    ...
    TaskScheduler.prototype.createEvaluatedTaskPollerSubscription = function(evaluateGraphStream) {
        ...
        .takeWhile(self.isRunning.bind(self))
        .subscribe(
            evaluateGraphStream.onNext.bind(evaluateGraphStream, { graphId: null }),
            self.handleStreamError.bind(self, 'Error polling for tasks')
        );
    };
