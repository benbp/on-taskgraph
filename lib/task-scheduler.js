...
function taskSchedulerFactory(
    ...
    function TaskScheduler(options) {
        ...
        process.on('SIGINT', function() {
            //console.log(self.scheduledTasks);
            console.log(self.scheduledTasks.length, ' scheduled');
            console.log('null graphIds = ', self.nullCnt);
        });
    }
    ...
    TaskScheduler.prototype.handleGraphFinishedEvent = function(data) {
        ...
        .flatMap(function(data) {
            if (!data.graphId) {
                self.nullCnt +=1;
                return Rx.Observable.empty();//self.findPotentialFinishedGraphs(data);
            } else {
                return Rx.Observable.just(data);
            }
        })
