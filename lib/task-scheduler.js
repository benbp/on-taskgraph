...
function taskSchedulerFactory(
    ...
    TaskScheduler.prototype.initializePipeline = function() {
        var tasksToSchedule = this.evaluteGraphStream
                ...
                .flatMap(this.graphobjects.checkoutTask.bind(this.store, this.schedulerId))
    ...
    TaskScheduler.prototype.evaluateGraph = function(graphId) {
        var self = this;
        return self.store.checkGraphDone(graphId)
        .then(function(graph) {
            if (graph) {
                return self.store.findReadyTasksForGraph(graphId);
            } else {
    ...
    TaskScheduler.prototype.handleStartTaskGraphRequest = function(name, options, target) {
        ...
        .then(function(graph) {
            return self.graphobjects.persistGraphObject(graph.toJSON());
        })
        .then(function(graph) {
            debugger;
    ...
    TaskScheduler.prototype.start = function() {
        this.graphobjects = store.getGraphObjectStore();
